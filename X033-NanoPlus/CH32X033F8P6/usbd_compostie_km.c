...
void USART2_Receive_Handle( void )
{
    ...
    if( flag == 0 )
    {
        ...
        if( ( USART_Recv_Dat == DEF_KEY_CHAR_A ) ||
            ...
            ( USART_Recv_Dat == DEF_KEY_CHAR_ENTER ) )
        {
...
void KB_Scan( void )
{
    ...
    if( ( scan_cnt % 10 ) == 0 )
    {
        ...
        if( scan_result == ( GPIO_ReadInputData(GPIOB) & 0x00F0 ) )
        {
            KB_Scan_Done = 1;
    ...
    else if( ( scan_cnt % 5 ) == 0 )
    {
        ...
        scan_result = GPIO_ReadInputData(GPIOB) & 0x00F0 ;
    }
}
                ...
                if( ( KB_Scan_Result & ( 1 << i ) ) != ( KB_Scan_Last_Result & ( 1 << i ) ) )
                {
                    ...
                    if(( KB_Scan_Result & ( 1 << i ) ) )           // Key press
                    {
                        ...
                        else if( i == 7 )
                        {
                            for( j = 2; j < 8; j++ )
                            {
                                if( KB_Data_Pack[ j ] == DEF_KEY_CHAR_ENTER )
                                {
                                    LED_OFF();
                                    break;
                                }
                    ...
                    else                                            // Key release
                    {
                        ...
                        {
                            LED_ON();
                            KB_Data_Pack[ 2 + key_cnt ] = DEF_KEY_CHAR_ENTER;
                        }
                        key_cnt++;
                    }
                }
...
void MS_Scan_Init( void )
{
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14 | GPIO_Pin_15 | GPIO_Pin_16 | GPIO_Pin_17;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init( GPIOA, &GPIO_InitStructure );
}
...
void MS_Sleep_Wakeup_Cfg( void )
{
    ...
    GPIO_EXTILineConfig( GPIO_PortSourceGPIOA, GPIO_PinSource16 );
    ...
    GPIO_EXTILineConfig( GPIO_PortSourceGPIOA, GPIO_PinSource17 );
    ...
    EXTI->INTENR |= EXTI_INTENR_MR14 | EXTI_INTENR_MR15 | EXTI_INTENR_MR16 | EXTI_INTENR_MR17;
}
...
void MCU_Sleep_Wakeup_Operate( void )
{
    ...
    EXTI_ClearFlag( EXTI_Line14 | EXTI_Line15 | EXTI_Line16 | EXTI_Line17 );
    ...
    else if( EXTI_GetFlagStatus( EXTI_Line14 | EXTI_Line15 | EXTI_Line16 | EXTI_Line17 ) != RESET )
    {
        EXTI_ClearFlag( EXTI_Line14 | EXTI_Line15 | EXTI_Line16 | EXTI_Line17 );
        USBFS_Send_Resume( );
    }
